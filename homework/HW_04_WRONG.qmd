...
Reliability seems to drop wildly after a depth of 5, so that seems to be a reasonable cap.

6. Fit a regression tree to the full data using the maximum tree depth you chose in the previous question.
```{r}
tree2 <- rpart(total_deaths_per_100k ~ EP_POV150 + EP_UNEMP + EP_HBURD + EP_NOHSDP + EP_UNINSUR + EP_AGE65 + EP_AGE17 + EP_DISABL + EP_SNGPNT + EP_LIMENG + EP_MINRTY + EP_MUNIT + EP_MOBILE + EP_CROWD + EP_NOVEH + EP_GROUPQ + EP_NOINT, data = trim, maxdepth = 5)
```
